...
- **VPC-SC対応のためのフロントエンド刷新**: VPC-SCの完全な保護下で運用するため、GASとGoogleフォームから脱却し、AppSheet（Enterprise）やIAPで保護された内部Webアプリを申請フロントエンドとして導入する。管理帳票はConnected Sheetsを利用したスプレッドシートを維持する。
...
- 認証は、OIDC（OpenID Connect）トークン認証をサポートします。Cloud Schedulerからのバッチ処理や、GASからのリクエスト（IAM Credentials APIによる権限借用）をセキュアに認証します。
...
- **Pull Request**: Pull Requestを作成すると、PythonとTerraformの静的解析・テストに加え、**`tfsec`によるTerraformのセキュリティ脆弱性スキャン**と `terraform plan` が自動実行されます。`plan`の結果はコメントとして投稿され、マージ前にセキュリティリスクとインフラ変更内容を確認できます（※導入初期のため、現在はマージをブロックしない `soft_fail` モードで稼働しています）。
...
### 11.10. 環境の安全な削除 (Terraform Destroy)

不要になった環境を削除する際、本システムは監査証跡の消失や他システムへの影響を防ぐため、安全に `terraform destroy` が実行できるよう設計されています。

#### 仕様と注意点

- **APIは無効化されません:** Terraformで管理されているAPI（`google_project_service`）には `disable_on_destroy = false` と `prevent_destroy = true` が設定されています。そのため、`destroy` を実行してもプロジェクトのAPIが無効化され、他のシステムが予期せず停止する事故は起きません。
- **監査ログテーブルは保護されています:** BigQueryのデータセットやテーブル（`iam_access_requests`、`iam_access_change_log` など）には `prevent_destroy = true` が設定されています。誤って実行しても監査データは消えません。
...
1. **安全な部分削除（推奨）:**
   ...
   このコマンドを実行すると、Cloud Run、Cloud Scheduler、アラートポリシーなどの「コンピュート/設定リソース」のみが削除されます。BigQueryテーブルやAPIリソースの削除フェーズで `prevent_destroy` によるエラーが発生して処理が停止しますが、**これが想定された安全な挙動です**。

1. **監査ログも含めた完全削除（特例）:**
   テスト環境の片付けなどでBigQueryの監査データもすべて消し去りたい場合は、事前にコード上でライフサイクル保護を解除する必要があります。

   - `terraform/main.tf` 内の `google_project_service.services` の `prevent_destroy = true` をコメントアウト。
   - `terraform/modules/bigquery/main.tf` 内のデータセットおよび全テーブルの `prevent_destroy = true` をコメントアウト。
   - その後、再度 `terraform destroy` を実行してください。（※この場合でも `disable_on_destroy = false` が効いているため、APIは無効化されません）
...
**方針A: VPC-SCを有効化し、GASを完全に捨てる（エンタープライズ特化）**
VPC-SCによるネットワーク防御を最優先とする場合、パブリックIPで動くGASやGoogleフォームをインターフェースにすることは諦めます。
代わりに、AppSheet（Enterpriseプラン） や、VPCネットワーク内にデプロイした 内部向けWebアプリ（Identity-Aware Proxyで保護） を申請フロントエンドとして作り直します。

なお、フロントエンドを移行した場合でも、管理帳票（出力・可視化）としては Google スプレッドシート を残すことが推奨されます。Connected Sheets（BigQueryデータコネクタ）機能を利用すれば、GASを経由せずにVPC-SC環境下でも安全かつ直接BigQueryのビュー（`v_sheet_*`）をRead-Onlyで同期・表示可能です。
