...
| テーブル名 | 更新方式 | 真の利用目的（プログラムの実態） | データの使い道 |
| :--- | :--- | :--- | :--- |
| **`iam_access_requests`** | 追記＋更新 | **申請・承認の正本。** ユーザーがSaaSポータルから送信したリクエストの最新ステータス（PENDING, APPROVED 等）を管理する。GASやCloud Runによってステータスが随時 UPDATE される。 | **・実行エンジンの入力**: 承認された申請をCloud Runが読み取り、実際のIAM付与・剥奪処理を行うためのキューとして機能。 <br>**・有効期限管理**: 期限付き権限の失効日を管理し、自動剥奪バッチの対象を特定するために利用。 |
...
| **`iam_role_master`** | 追記/手動更新 | **IAMロールの日本語名マスタ。** 新規ロールを自動検知してGeminiが翻訳・追記。誤訳は運用者が手動でUPDATEする。 | **・UI/UXの統一**: 帳票上の英名ロールを分かりやすい日本語で表示する。 |
...
- **利用目的:** IAMロールの英語IDと日本語表示名のマッピングを管理する。Geminiによる自動翻訳結果を保持し、運用者による手動修正も可能。
- **主要なソース:** `sql/004_workbook_tables.sql`, `cloud-run/app/main.py` (/jobs/discover-iam-roles)

| カラム名 | 型 | NULL | 説明 |
| :--- | :--- | :--- | :--- |
| `role_id` | STRING | NOT NULL | 正式なロールID (例: roles/viewer) |
| `role_name_ja` | STRING | NULLABLE | 日本語表示名（Geminiによる推論、または手動設定） |
| `is_auto_translated` | BOOL | NOT NULL | Geminiによって自動翻訳されたままの状態か（手動確認済みか）のフラグ |
| `updated_at` | TIMESTAMP | NOT NULL | レコードの最終更新日時 |
