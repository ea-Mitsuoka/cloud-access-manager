...
- **動作:** システムは組織配下の全プロジェクトを管理対象とします。リソースの棚卸しやGoogleグループの収集を行いますが、**IAM権限の変更はセキュリティの観点からプロジェクトレベルに限定**されます。
...
#### 9.2 環境の削除とテーブルの保護 (Teardown)

不要になった環境を削除する場合、監査ログの保護要件に合わせて以下のいずれかの手順を実施してください。

**1. 安全な削除（推奨: コンピュートリソースのみ削除）**
Cloud RunやCloud Schedulerなどのインフラのみを削除し、監査ログ（BigQuery）を安全に残す場合は、提供されている専用スクリプトを使用します。
...
## 11. トラブルシューティング（詳細シナリオ別）

本番運用中に発生しうる代表的なエラーシナリオと、その復旧手順・一次対応のガイドラインです。

### 11.1 GASからCloud Runへの連携エラー (401 Unauthorized / OIDC Token Error)

- **症状:** スプレッドシートで「承認済」に変更しても権限が付与されず、GASの実行ログに `Failed to get OIDC token from IAM Credentials API` が記録されるか、Cloud Run側で `401 Unauthorized` が発生する。
- **原因:** GASのトリガー実行ユーザーが、OIDCトークンを生成するための権限を持っていないか、Audience（送信先URL）の設定が不一致を起こしています。
- **対応:**
  1. `saas.env` の `GAS_TRIGGER_OWNER_EMAIL` にGASトリガーを設定したユーザーのメールアドレスが正しく指定され、`terraform apply` が完了しているか確認してください（これにより `roles/iam.serviceAccountTokenCreator` が付与されます）。
  1. Apps Scriptのスクリプトプロパティ `CLOUD_RUN_EXECUTE_URL` の値が、Terraformが出力したCloud RunのURLと完全に一致しているか確認してください。
  1. **【重要】** `saas.env` で `enable_vpc_sc=true` と設定し VPC-SC を有効化している場合、パブリックIPで動くGASからのリクエストは境界で完全に遮断されます。詳しくは「5. 高度なセキュリティ設定 (VPC-SC)」を参照してください。

### 11.2 BigQuery APIのクォータ超過 (Rate Limit / Quota Exceeded)

- **症状:** Cloud Runのログやバッチ実行レポート（`iam_pipeline_job_reports`）に `403 Quota exceeded` や `rateLimitExceeded` が記録される。
- **原因:** 短期間に大量のデータ更新が発生したか、BigQueryプロジェクト全体のAPI呼び出し上限に達しました。
- **対応:**
  1. 本システムは「バルク一括更新」や「ストリーミング挿入」によりAPIのコール数を極限まで最適化していますが、同プロジェクトに相乗りしている他システムのリソース消費に巻き込まれた可能性があります。
  1. GCPコンソールの「IAMと管理 > クォータとシステム制限」から BigQuery API の消費状況を確認し、必要に応じて上限引き上げを申請してください。
  1. 期限切れ権限の自動剥奪ジョブなどは、一時的なエラーで失敗した場合でも翌日のバッチで自動的にリトライ（自己修復）されるため、軽微な超過であれば手動リカバリは不要です。

### 11.3 存在しないはずのインシデントアラートが届く (Cloud Asset API の遅延)

- **症状:** 権限を正しく承認・付与した直後、または手動で権限を剥奪した直後に、`APPROVED_NOT_APPLIED`（付与されていない）や `REJECTED_BUT_EXISTS`（残存している）のアラートが誤発報される。
- **原因:** GCPの Cloud Asset API のインデックス反映には、数分〜数時間程度の遅延（Eventual Consistency）が存在します。
- **対応:**
  1. 本システムは誤報（狼少年化）を防ぐため、「ステータス変更から12時間は猶予期間（Grace Period）としてアラート発報を保留する」保護ロジックが組み込まれています。そのため、通常の運用でこの現象は発生しません。
  1. もし12時間を超えてアラートが鳴り続ける場合、実際にCloud RunによるIAM付与が失敗している可能性があります。`iam_access_change_log` テーブルのエラー内容を確認し、必要に応じてGCPコンソールから手動でIAM権限を是正してください。
...
- **対応:**
  ...
  1. 対応が完了し権限が不要になったと判断した時点で、管理者は速やかにGCPコンソールから該当の権限を手動で剥奪してください。
  1. 手動で剥奪された権限は、翌日のデータ収集バッチによってシステムに認知され、自動的にインシデント管理から外れます。

## 12. 管理帳票（スプレッドシート）のセットアップ手順
...
### 12.2. Connected Sheets による監査データの取り込み（可視化）
...
### 12.3. 「IAM権限設定マトリクス」の作成（ピボットテーブル）
...
### 13.1 月額費用の見積もり目安（月間300件の申請を想定）

| GCPサービス | 役割 | 想定利用量と料金概算（月額） |
| :--- | :--- | :--- |
| **Cloud Run** | 実行エンジン・バッチ処理 | **約 0 円**<br>月間数千秒の実行時間。GCPの無料枠に完全に収まります。 |
| **Cloud Scheduler** | 日次バッチのトリガー | **約 50円 ($0.30)**<br>無料枠（3ジョブ/月）を超える3ジョブ分について、1ジョブあたり$0.10かかります。 |
