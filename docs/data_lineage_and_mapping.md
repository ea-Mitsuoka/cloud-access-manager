...
1. **入力フォーマットの自動補完によるエラーの防止（バリデーションの壁）**
   - 以前のGoogleフォーム環境では、ユーザーが「対象リソース」に `my-project` とだけ書いた場合にGCP APIの厳密なフォーマット（`projects/my-project`）と一致せずエラーになるリスクがありました。現在はこの問題を解消するため、GASのバックエンド（`Code.gs` の `normalizeResourceName_` 関数）にて、プレフィックスが不足している場合に自動で `projects/` を補完する強力な防御層を設けています。
