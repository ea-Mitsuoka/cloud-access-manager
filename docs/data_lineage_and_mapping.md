# IAM Access Manager データリネージとマッピング定義

本ドキュメントは、IAM Access Managerにおける全13テーブル・全カラムの因果関係（データリネージ）を定義したものです。
情報の発生源である **GCP API** や **Googleフォーム（UI）** から、加工・保存を担う **GAS/Python**、そして最終的な監査要件である **スプレッドシート帳票** に至るまでのデータの血統を網羅しています。
...
- **ビジネス要件 (Google Forms & Sheets)**
  - **役割:** ユーザーが権限を要求し、管理者が承認・棚卸しを行うインターフェース。
  - **要求される項目:** `対象プリンシパル`, `対象リソース`, `付与・変更ロール`, `申請理由・利用目的`, `利用期限`, `承認者メール`。
- **GAS (`Code.gs`)**
  - **役割:** フォームの日本語フィールドを抽出し、システムが解釈できる英語キーのJSONペイロードにマッピングします。
  - **マッピング:** `対象リソース` -> `resource_name`、`申請理由・利用目的` -> `reason`。これを Cloud Run の `/api/requests` へ送信します。
...
| 概念 | ① GCP API (現実) | ② フォーム/GAS (意図) | ③ Python/BQ (正本) | ④ 履歴結合 (SQL) | ⑤ スプレッドシート (UI) |
| :--- | :--- | :--- | :--- | :--- | :--- |
...
| `request_type` | フォーム「申請種別」 | GASで `GRANT/REVOKE/CHANGE` に正規化 | 実行エンジンのアクション判定 (`_normalize_action`) |
| `principal_email` | フォーム「対象プリンシパル」 | GASからそのまま送信 | IAM付与対象、履歴および不整合検知の結合キー |
| `resource_name` | フォーム「対象リソース」 | GASからそのまま送信 | IAM付与対象、履歴および不整合検知の結合キー |
| `role` | フォーム「付与・変更ロール」| GASからそのまま送信 | IAM付与対象、履歴および不整合検知の結合キー |
| `reason` | フォーム「申請理由」 | 緊急時はプレフィックス `[緊急]` を付与 | 履歴スナップショット、監査時のレビュー根拠 |
| `expires_at` | フォーム「利用期限」 | GASでパースし、指定日の `23:59:59.999` に変換 | 自動剥奪バッチ (`/revoke_expired_permissions`) の対象判定 |
| `requester_email` | フォーム「申請者メール」 | GASからそのまま送信 | 申請履歴イベントの記録 |
| `approver_email` | フォーム「承認者」 | GASからそのまま送信 | 申請履歴イベントの記録 |
...
| `event_type` | 処理のコンテキスト | フォーム送信時は `REQUESTED`、以降は `STATUS_CHANGED` | 履歴のフィルタリング |
...
| `actor_source` | 処理のコンテキスト | `FORM_SUBMIT`, `SHEET_EDIT`, `SYSTEM_BATCH` 等 | どこから操作されたかの監査 |
| `event_at` | 処理日時 | 処理実行時のタイムスタンプ | 時系列ソート |
| `details` | 処理のコンテキスト | シート名や詳細メモをJSONで格納 | 監査補助 |
...
Cloud Identity APIから収集したGoogleグループとそのメンバーの一覧です。

| カラム名 | 源泉 (Source) | 加工・挿入 (Transform/Load) | 下流での用途 (Destination) |
| :--- | :--- | :--- | :--- |
| `group_email` | Cloud Identity API | グループの `groupKey.id` | メンバーシップとの結合キー、帳票表示 |
| `group_name` | Cloud Identity API | グループの `displayName` | 帳票表示 (`v_sheet_group`) |
| `description` | Cloud Identity API | グループの `description` | 帳票表示 (`v_sheet_group`) |
...
- **`principal_catalog`**: `iam_policy_permissions` の情報を `MERGE` し、メールアドレスごとの種別を最新に保つマスタ。
...
## 4. 監査と精査のポイント（アーキテクチャ上のリスク）

このリネージマップを踏まえ、データエンジニアとして精査すべきポイントは以下の通りです。

1. **入力の自由度とAPIの厳格さの不一致（バリデーションの壁）**
   - ユーザーはフォームの「対象リソース」に `my-project` とだけ書く可能性がありますが、GCP API (実行層の `iam_executor.py` や収集層) は `projects/my-project` という厳密なフォーマット（`resource_name`）を要求します。現状、GASやPython側でこの接頭辞を自動補完する強力なバリデーションが存在しない場合、実行時エラーになるリスクがあります。
