...
## 💡 本検討の背景：スプレッドシート×BigQuery構成へのVPC-SC適用

本アーキテクチャを比較考察するにあたり、出発点となったのは **「スプレッドシートによる承認管理（UI/UX）」と「BigQueryによる改ざん不可能な監査ログ」という既存の強力な組み合わせを維持したまま、システム全体をVPC Service Controls（VPC-SC）で保護したい** という高度なセキュリティ要件でした。

もし「スプレッドシートでの管理を諦める」という決断が許されるのであれば、すべてのUI（申請・承認・棚卸し）を単一のWebアプリケーションとしてフルスクラッチ開発し、VPC-SCの閉域網内に完全に閉じ込める案も検討可能でした。しかし、現場の管理者が慣れ親しんだ「複数行のコピペ処理」や「一目で全体を把握できる一覧性」といった圧倒的な業務効率を、セキュリティのために犠牲にすることは本末転倒であると判断し、スプレッドシートの完全排除案は不採用としました。

この「譲れない部分（スプレッドシート）」を死守しつつ、パブリックインターネットからアクセスされるGoogle Workspace（GAS）と、VPC-SCの強固な境界内にあるCloud Run/BigQueryをいかにして安全に連携させるか。そして、将来のSaaS展開を見据えた拡張性をどう確保するか。

その解決策を模索した結果として以下の4つのアーキテクチャ案が浮上し、導入の手軽さと将来性の総合的な評価の結果として **「第2案（IAP ＋ Cloud Run）」が採用されるに至りました**。
